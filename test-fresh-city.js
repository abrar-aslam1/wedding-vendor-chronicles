// Test with a completely fresh city that has no cache entries
...
async function testFreshCity() {
  console.log('🧪 Testing Fresh City with Subcategories...');
  
  try {
    const testCity = 'FreshTestCity';
    const testState = 'TX';
    ...
    console.log(`\n1️⃣ Testing API call with wedding subcategory for ${testCity}...`);
    ...
    if (apiError1) {
      console.error('❌ API Error (wedding):', apiError1);
      return;
    }
    
    console.log(`✅ API Success (wedding): ${apiData1?.results?.length || 0} results`);
    ...
    console.log(`\n2️⃣ Testing API call with portrait subcategory for ${testCity}...`);
    ...
    if (cacheEntries && cacheEntries.length > 0) {
      cacheEntries.forEach((entry, i) => {
        ...
        if (entry.subcategory === 'wedding') {
          console.log(`   ✅ WEDDING ENTRY FOUND!`);
        } else if (entry.subcategory === 'portrait') {
          console.log(`   ✅ PORTRAIT ENTRY FOUND!`);
        }
      });
      ...
      if (uniqueSubcategories.length >= 2) {
        console.log('🎉 SUCCESS: Multiple subcategories are being cached separately!');
      } else {
    ...
    console.log(`\n4️⃣ Testing cache retrieval for wedding subcategory...`);
    ...
    if (apiError3) {
      console.error('❌ API Error (wedding cache test):', apiError3);
    } else {
