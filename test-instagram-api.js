// Test script to verify the search-instagram-vendors API is working
const testInstagramSearchAPI = async () => {
  ...
  console.log('🧪 Testing search-instagram-vendors API...');
  console.log('📦 Test payload:', testPayload);
  
  try {
    const response = await fetch(`${supabaseUrl}/functions/v1/search-instagram-vendors`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      if (data.results && data.results.length > 0) {
        console.log('📊 First result:', {
          ...
          instagram_handle: data.results[0].instagram_handle,
          follower_count: data.results[0].follower_count,
