...
/**
 * OPTIMIZED Comprehensive Vendor Collection Script
 * 
 ...
 * 
 * OPTIMIZED Features:
 * - Proper DataForSEO rate limiting (2000 req/min vs 10 req/min)
 * - Concurrent requests (up to 30 simultaneous)
 * - Removed artificial daily limits
 * - Added Wedding Decorators category
...
// OPTIMIZED Rate limiting configuration based on DataForSEO limits
const REQUESTS_PER_MINUTE = 1800; // Conservative but much higher (DataForSEO allows 2000)
const MAX_CONCURRENT_REQUESTS = 25; // Conservative (DataForSEO allows 30)
const DELAY_BETWEEN_BATCHES = 100; // Small delay between batches (ms)

// Progress tracking
const PROGRESS_FILE = './vendor-collection-progress-optimized.json';
...
// Wedding vendor categories and subcategories (UPDATED with Wedding Decorators)
...
class OptimizedVendorCollector {
  constructor() {
  ...
  async checkIfCached(city, state, category, subcategory) {
    try {
      ...
      if (data) {
        console.log(`✅ Cache hit: ${city}, ${state} - ${category} (${subcategory || 'none'})`);
        return true;
      }
  ...
  async callVendorAPI(city, state, category, subcategory) {
    return new Promise(async (resolve) => {
      try {
        ...
        console.log(`🔍 API Call: ${category} in ${location} (${subcategory || 'none'})`);
        ...
        if (error) {
          console.error(`❌ API Error: ${location} - ${category}:`, error);
        ...
        console.log(`✅ Success: ${resultCount} results for ${category} in ${location} (Cost: $${cost})`);
        ...
        console.error(`❌ Network error: ${city}, ${state} - ${category}:`, error);
        this.stats.failedRequests++;
        resolve(false);
      }
    });
  ...
  async manageRateLimit() {
    ...
    // If we're at the rate limit, wait until the next minute
  ...
  printStats() {
    ...
    console.log('\n📊 OPTIMIZED COLLECTION STATISTICS');
    console.log('==================================');
    ...
    console.log(`Requests this minute: ${this.stats.requestsThisMinute}/${REQUESTS_PER_MINUTE}`);
    console.log(`Estimated Time Remaining: ${estimatedTimeRemaining.toFixed(1)} minutes`);
    console.log('==================================\n');
  }

  async run() {
    console.log('🚀 Starting OPTIMIZED Comprehensive Vendor Collection');
    console.log(`📍 Collecting from ${Object.keys(MAJOR_CITIES).length} states`);
    console.log(`🏪 ${VENDOR_CATEGORIES.length} vendor categories (including Wedding Decorators!)`);
    ...
    console.log(`🔄 Max concurrent: ${MAX_CONCURRENT_REQUESTS} requests`);
    console.log(`💰 No daily limits - full speed ahead!\n`);
    ...
    console.log(`📋 Found ${allRequests.length} pending requests to process\n`);
    ...
    for (const request of allRequests) {
      const isCached = await this.checkIfCached(
        request.city, 
        request.state, 
        request.category, 
        request.subcategory
    ...
    console.log(`💾 Skipped ${allRequests.length - uncachedRequests.length} cached requests`);
    console.log(`🚀 Processing ${uncachedRequests.length} new requests\n`);

    if (uncachedRequests.length === 0) {
      console.log('🎉 All requests are already cached!');
      this.printStats();
      return;
    }
...
// Main execution
async function main() {
  const collector = new OptimizedVendorCollector();
...
export { OptimizedVendorCollector };
