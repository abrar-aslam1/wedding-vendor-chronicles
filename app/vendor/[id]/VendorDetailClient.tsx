...
import { ArrowLeft, Heart, Clock, DollarSign, MapPin, Users } from "lucide-react";
...
const VendorDetailClient = ({ vendorId }: VendorDetailClientProps) => {
  const router = useRouter();
  const { toast } = useToast();
  const [vendor, setVendor] = useState<SearchResult | null>(null);
        ...
        <div className="container mx-auto px-4 py-8 mt-16">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              {vendor.category && (
                <BreadcrumbItem>
                  <BreadcrumbLink href={routes.category(vendor.category)}>
                    {vendor.category}
                  </BreadcrumbLink>
                </BreadcrumbItem>
              )}
