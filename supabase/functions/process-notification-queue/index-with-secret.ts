...
serve(async (req) => {
  ...
  try {
    ...
    // Check authentication - either proper headers OR secret parameter
    const hasValidAuth = req.headers.get('authorization') || req.headers.get('apikey');
    const hasValidSecret = secret === Deno.env.get('CRON_SECRET') || secret === 'your_secret_key_here_change_this';
    ...
    return new Response(
      JSON.stringify({ 
        ...
        authMethod: hasValidAuth ? 'headers' : 'secret'
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200 
